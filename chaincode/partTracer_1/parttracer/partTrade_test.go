...
func TestSellPart(t *testing.T) {
	...
	// Set the transientData to the MockStub
	m := make(map[string][]byte)
	b := []byte("{\"partId\":\"engine_1\",\"dealPrice\":9999}")
	m["PartDealData"] = b
	...
	// Set Transient depends on signedproposal to be initialized and that is being done in MockTransactionStart
	err = mockStub.SetTransient(m)

	if err != nil {
		assert.FailNow(t, "Failure during setup", err.Error())
	}

	part, err = contract.SellPart(ctx, "engine_1", "", "privacy_v1")
	mockStub.MockTransactionEnd(txID)
	assert.Nil(t, err, "should not error when query part")
	assert.Equal(t, "engine_1", part.PartID, "should update the PartID of the Part")
	assert.Equal(t, "engine", part.PartName, "should update the PartName of the Part")
	...
	// Part Does not exist
	mockStub.MockTransactionStart(txID)
	part, err = contract.SellPart(ctx, "properller_2", "", "privacy_v1")
	mockStub.MockTransactionEnd(txID)
	...
	// Invalid PartID Case
	mockStub.MockTransactionStart(txID)
	part, err = contract.SellPart(ctx, "", "", "privacy_v1")
	mockStub.MockTransactionEnd(txID)
	assert.Nil(t, part, "should return nil part on Sell")
...
func TestGetDealData(t *testing.T) {

	var err error
	var part *Part
	var partDealData *PartDealData
	var partID string
	...
	if err != nil {
		assert.FailNow(t, "Failure during seting TransientData", err.Error())
	}

	part, err = contract.SellPart(ctx, "engine_1", "", "privacy_v1")
	mockStub.MockTransactionEnd(txID)
	assert.Nil(t, err, "should not error when sell part")
	assert.Equal(t, "engine_1", part.PartID, "should update the PartID of the Part")
	assert.Equal(t, "engine", part.PartName, "should update the PartName of the Part")
	assert.Equal(t, "Two seater Private Plane", part.Description, "should update the Description of the Part")
	assert.Equal(t, uint32(10000), part.QuotePrice, "should update the QuotePrice of the Part")
	...
	partDealData, err = contract.QueryPartDealPrice(ctx, "engine_1", "privacy_v1")
	assert.Nil(t, err, "should not error when query deal data")
	assert.Equal(t, "engine_1", partDealData.PartID, "should update the PartID of the Deal")
	assert.Equal(t, uint32(9999), partDealData.DealPrice, "should update the PartID of the Deal")
