...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name partTracer --version ${VERSION} --init-required --package-id ${PACKAGE_ID} --sequence ${VERSION} ${PRIVATE_COLLECTION_POLICY} >&log.txt
  set +x
  cat log.txt
...
checkCommitReadiness() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name partTracer --version ${VERSION} --sequence ${VERSION} --output json --init-required ${PRIVATE_COLLECTION_POLICY}>&log.txt
    res=$?
    set +x
    let rc=0
    for var in "$@"
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name partTracer $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} ${PRIVATE_COLLECTION_POLICY} --init-required >&log.txt
  res=$?
  set +x
  cat log.txt
...
invokeSellPart(){
  ...
  set -x
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n partTracer $PEER_CONN_PARMS -c '{"function":"sellPart","Args":["engine_1","Org2MSP"]}' --transient "{\"PartPrivateData\":\"$DEAL_PRICE\"}" >&log.txt
  res=$?
  set +x
  cat log.txt
  verifyResult $res "Invoke execution on $PEERS failed "
